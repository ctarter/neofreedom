...
class gdpol_obj_poll {
    ...
    public function show_form($user_id = 0) {
    ...
    public function show_choices($user_id = 0) {
        $user_id = $this->user_id($user_id);        
    ...
    public function show_results($user_id = 0) {
        $user_id = $this->user_id($user_id);        
    ...
    public function is_allowed_to_vote($user_id = 0) {
        $user_id = $this->user_id($user_id);
    ...
    public function has_voted($user_id = 0) {
        $user_id = $this->user_id($user_id);

        return gdpol_db()->user_voted_in_poll($this->id, $user_id);
    }

    public function has_replied($user_id = 0) {
        $user_id = $this->user_id($user_id);
    ...
    public function admin_render_settings() {
    ...
    public function admin_render_results() {
        $results = $this->results();
    ...
    public function render_message($user_id = 0) {
        ...
        if ($this->mode == 'voted') {
            ...
            $cde = 'error';
        } else if ($this->is_open() && !$this->is_allowed_to_vote($user_id) && $this->respond == 'reply') {
    ...
    public function actions($user_id = 0) {
        $actions = array();
    ...
    public function vote($user_id = 0) {
        $user_id = $this->user_id($user_id);
    ...
    public function results() {
        ...
        foreach ($this->responses as $r) {
            $answer = absint($r['id']);
            $counts = isset($db['counts'][$answer]) ? $db['counts'][$answer] : 0;
            $voters = isset($db['voters'][$answer]) ? $db['voters'][$answer] : array();
    ...
    private function _to_meta_input() {
        $keys = array('responses', 'status', 'respond', 'choice', 'choice_limit', 
            'close', 'close_users', 'close_datetime', 'show', 'removal');
